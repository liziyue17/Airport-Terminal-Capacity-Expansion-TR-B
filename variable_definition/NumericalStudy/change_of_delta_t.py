...
if __name__ == '__main__':
    for i in range(len(delta_t_list)):
        print("Current trial: ", i+1, " total trials: ", len(delta_t_list))
        cache_data_file = cache_data_folder + 'F_mat_delta_t_ ' + str(delta_t_list[i]) + '.csv'
        check_folder(result_folder)
        check_folder(cache_data_folder)

        F_array, opt_delta_K_array = get_F_function_result(q_array, delta_K_array, delta_t_list[i], dt, T, q_max, eta, sigma, A,
        ...
        if status != -1:
            ...
            plot_V_F_function_old(q_array, V_array, F_array, plot_title, opt_trigger_point=opt_trigger_point,
                              NPV_point=NPV_point, result_folder=result_folder, filename='solve_V_F_delta_t_ ' + str(delta_t_list[i]) + '.pdf')
        else:
